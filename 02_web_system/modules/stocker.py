...
class Stocker():
    ...
    def __init__(self, stock):
        ...
        # s = price
        # stock = pd.DataFrame(
        #     {'Date': s.index, 'y': s, 'ds': s.index, 'Close': s, 'Open': s}, index=None)
        ...
        self.most_recent_price = float(
            self.stock['y'].iloc[len(self.stock) - 1])
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(
                stat, self.stock[stat].iloc[len(self.stock) - 1], self.max_date.date()))
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.stock[self.stock['Date'] > str(dtt.strptime(
            self.max_date, '%Y-%m-%d').date() - relativedelta(years=self.training_years))]
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        stock_history = self.stock[self.stock['Date'] > str(dtt.strptime(
            self.max_date, '%Y-%m-%d').date() - relativedelta(years=self.training_years))]
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=None):
        ...
        if not nshares:
            ...
            print('\nPredicted price on {} = ${:.2f}.'.format(
                max(future['ds']), future['yhat'].iloc[len(future) - 1]))
            print('Actual price on    {} = ${:.2f}.\n'.format(
                max(test['ds']), test['y'].iloc[len(test) - 1]))
        ...
        # If a number of shares is specified, play the game
            ...
            for i, correct in enumerate(test_pred_increase['correct']):
                ...
                if correct == 1:
                    prediction_profit.append(
                        nshares * test_pred_increase['real_diff'].iloc[i])
            ...
            print('The Buy and Hold strategy profit =         ${:.2f}.'.format(
                float(test['hold_profit'].iloc[len(test) - 1])))
